...
const StreamOverlay = () => {
  ...
  // Scale 1920x1080 canvas to fit viewport
  ...
  // Detect mobile (only for very small screens like phones)
  useEffect(() => {
  ...
  const fetchNext = useCallback(async () => {
    try {
      ...
      if (data.status === "new" || data.status === "displaying") {
        ...
        // Resolve photo URL from storage path if needed
        ...
        // Calculate remaining time if already displaying
  ...
  // ─── COMPLETE DISPLAY (advance queue) ───
  const completeDisplay = useCallback(async () => {
    ...
    // Wait for fade-out animation, then fetch next
    setTimeout(() => {
      setCurrentItem(null);
      setState("loading");
      fetchNext();
        ...
        {currentItem && (
          ...
          >
            ...
            >
              ...
              >
                {currentItem.package_type === "premium"
                  ? "★ PREMIUM SUPPORTER ★"
                  : "SUPPORTER"}
              </p>
            </div>
            ...
            {/* Queue Position (bottom-left corner) */}
            <div
              style={{
                position: "absolute",
                bottom: "40px",
