...
const AdminDashboard = () => {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!user || user.role !== "admin") { api.setToken(null); navigate("/admin/login"); }
  ...
  const handleLogout = async () => { api.setToken(null); navigate("/admin/login"); };
              ...
              {[
                ...
                <div key={idx} className="bg-card border border-border rounded-xl p-6">
                  ...
                  {chart.pie ? (
                    ...
                    <ResponsiveContainer width="100%" height={160}>
                      <BarChart data={chart.data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(0, 0%, 16%)" />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="text"
                          readOnly
                          value={`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/payments/webhooks/stripe`}
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            navigator.clipboard.writeText(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/payments/webhooks/stripe`);
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        <code className="text-xs bg-secondary px-2 py-1.5 rounded text-primary font-mono flex-1 truncate">
                          {`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/merchandise/webhooks/printful`}
                        </code>
                        <button onClick={() => { navigator.clipboard.writeText(`${import.meta.env.VITE_API_URL || "http://localhost:3001"}/merchandise/webhooks/printful`); toast.success("Webhook URL copied!"); }}
